...
export default function Scheduler() {
  ...
  // Function to recalculate positions for all cards based on duration
  const recalculatePositions = async () => {
    try {
      // Group cards by assembler
      const cardsByAssembler = new Map<string, AssemblyCard[]>();
      
      assemblyCards.forEach(card => {
        if (card.assignedTo) {
          if (!cardsByAssembler.has(card.assignedTo)) {
            cardsByAssembler.set(card.assignedTo, []);
          }
          cardsByAssembler.get(card.assignedTo)?.push(card);
        }
      });
      ...
      for (const [assemblerId, cards] of Array.from(cardsByAssembler.entries())) {
        ...
        for (const card of sortedCards) {
          ...
          cumulativePosition += card.duration;
        }
      }
      
      toast({
        title: "Positions recalculated",
        description: "All card positions now reflect duration-based timeline",
      });
    } catch (error) {
      console.error('Error recalculating positions:', error);
      toast({
        title: "Error",
        description: "Failed to recalculate positions",
        variant: "destructive",
      });
    }
  };
  ...
  useEffect(() => {
    if (assemblyCards.length > 0) {
      ...
      for (const [, cards] of Array.from(cardsByAssembler.entries())) {
        ...
        for (const card of sortedCards) {
          if (Math.abs((card.position || 0) - expectedPosition) > 0.1) {
            needsRecalculation = true;
            break;
          }
          expectedPosition += card.duration;
        }
        if (needsRecalculation) break;
      }
      
      if (needsRecalculation) {
        console.log('Auto-recalculating positions to match duration-based logic');
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex items-center space-x-2">
                    <div className="bg-white text-red-500 rounded-full w-4 h-4 flex items-center justify-center text-[8px] font-bold border border-red-500">△</div>
                    <span className="text-[10px]">Dependency Conflict</span>
                  </div>
                </div>
              </div>
